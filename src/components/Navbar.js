...
export default function Navbar() {
    ...
    return (
        <nav id="top" className="font-nunito transition-colors duration-300 text-xl lg:text-2xl tracking-wider flex justify-between items-center p-10 text-black dark:text-white z-50 relative">
            ...
            <div className={`nav--right rounded-xl md:rounded-none bg-primary md:bg-transparent text-lg lg:text-xl gap-3 md:gap-12 absolute py-10 md:py-0 top-8 md:top-0 right-8 md:right-0 md:relative flex ${showNav ? "" : "hidden"} md:flex flex-col md:flex-row`}>
                ...
                <Link className=" pl-6 pr-12 md:px-0 transition hover:text-highlight md:hover-text-primary underline underline-offset-8 decoration-2 decoration-transparent hover:decoration-highlight md:hover:decoration-primary" to="/skills">skills</Link>
            ...
            <button id="nav--open" onClick={handleCLick} className={`absolute right-6 ${showNav ? "hidden" : ""} md:hidden stroke-2 stroke-black dark:stroke-white p-4`}>
                <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M13 18H20M4 6H20H4ZM4 12H20H4Z" />
                </svg>
            </button>
            <button id="nav--close" onClick={handleCLick} className={`absolute right-6 ${showNav ? "" : "hidden"} md:hidden fill-black dark:fill-white p-4`}>
                <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
