...
export default function Main(props) {
    ...
    return (
        <main className="project--main overflow-y-scroll flex flex-col items-center py-16 -mt-px font-nunito text-black dark:text-white bg-secondary dark:bg-gray-900 relative">
            <HashLink className="absolute left-32 top-10 rounded-md px-8 py-2 bg-white dark:bg-gray-800 tracking-wider" to={`/projects/#${props.data.id}`}>{"< back"}</HashLink>
            <h1 className="text-3xl font-black tracking-wider text-center">{props.data.title}</h1>
            <div className="flex items-center mt-8">
                <div className="bg-white dark:bg-gray-800 rounded-l-lg w-10 h-80"></div>
                <img className="rounded-lg w-96 shrink-0 shadow-md" src={props.data.preview} alt="" />
                <div className="bg-white dark:bg-gray-800 rounded-r-lg w-10 h-80"></div>
            </div>
            <div className="flex mt-8 max-w-xl">
                <p className="text-lg bg-white dark:bg-gray-800 px-10 py-5 mr-2 rounded-md">{props.data.description}</p>
                <div className="flex flex-col gap-y-2">
                    <a className="flex justify-center items-center px-2 h-full rounded-md uppercase tracking-wider text-sm bg-tertiary hover:bg-highlight stroke-white stroke-2 fill-transparent" href={props.data.live} target="_blank" rel="noreferrer">
                    ...
                    <a className="flex items-center justify-center px-2 h-full rounded-md uppercase tracking-wider text-sm bg-primary hover:bg-highlight fill-white" href={props.data.repo} target="_blank" rel="noreferrer">
