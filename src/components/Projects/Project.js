...
export default function Project(props) {
    ...
    if (stack.length) {
        if (props.data.featured) {
            ...
            return (
                <div id={props.data.id} className="flex items-center flex-col lg:flex-row xl:odd:ml-60 xl:even:mr-60 max-w-4xl">
                    <Link className="bg-primary shrink-0 rounded-lg overflow-hidden cursor-pointer" to={`/projects/${props.data.id}`}>
                        <img className="w-full sm:w-80 sm:h-80 opacity-50 pointer-events-auto saturate-0 hover:opacity-100 hover:saturate-100 transition" src={props.data.preview} alt="" />
                    </Link>
                    <div className="relative lg:-left-20 overflow-hidden max-w-2xl">
                        <div className="lg:mr-12 mt-4 lg:mt-0">
                            <h3 className="lg:text-right text-sm tracking-wider opacity-50">Featured Project</h3>
                            <Link to={`/projects/${props.data.id}`}>
                                <h2 className="lg:text-right text-xl lg:text-2xl font-black tracking-wider">{props.data.title}</h2>
                            </Link>
                        </div>
                        <div className="flex mt-4 gap-x-2">
                            <div className="flex items-center justify-center bg-white dark:bg-gray-800 py-3 lg:py-6 px-6 lg:px-8 shrink rounded-lg max-h-60 shadow-md">
                                <p>{props.data.description}</p>
                            </div>
                            <div className="flex flex-col gap-y-2 justify-start">
                                <a className="flex justify-center items-center shadow-md p-2 shrink-0 rounded-md bg-tertiary hover:bg-highlight stroke-white stroke-2 fill-transparent" href={props.data.live} target="_blank" rel="noreferrer">
                                ...
                                <a className="flex items-center justify-center shadow-md p-2 shrink-0 rounded-md bg-primary hover:bg-highlight fill-white" href={props.data.repo} target="_blank" rel="noreferrer">
                        ...
                        <div className="flex flex-wrap justify-start lg:justify-end gap-2 mt-2 lg:ml-20 lg:pl-2 mr-10 lg:mr-0 text-white">
                            {stackElements}
                        </div>
                    </div>
                    <div className="bg-black dark:bg-white opacity-20 w-full h-px mt-8 lg:hidden"></div>
                </div>
            )
        } else {
            const stackElements = props.data.stack.map(item => {
